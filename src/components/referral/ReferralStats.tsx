    ...
    <div>
      ...
      >
        <div className="flex items-center justify-between mb-2">
          <p className="text-xs text-white/40 uppercase tracking-wider">Your Referral Link</p>
          <span className="text-sm font-bold text-[#FFD700]">{stats?.referralCode}</span>
        </div>
        <div className="flex gap-2">
          <code
            className="flex-1 px-3 py-2 rounded-lg text-xs text-white/70 truncate select-all"
        ...
        <p className="text-[10px] text-white/25 mt-2">Share this link. When someone signs up, they become your referral.</p>
      </div>

      {/* Top stat cards */}
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="p-4 rounded-xl" style={{ background: 'rgba(33,150,243,0.06)', border: '1px solid rgba(33,150,243,0.12)' }}>
          <p className="text-xs text-white/40 mb-1">Total Referrals</p>
        ...
        <div className="p-4 rounded-xl" style={{ background: 'rgba(76,175,80,0.06)', border: '1px solid rgba(76,175,80,0.12)' }}>
          ...
          <p className="text-2xl font-bold text-[#4CAF50]">${(stats?.totalCommissionEarned ?? 0).toFixed(2)}</p>
          {stats && stats.pendingCommissions > 0 && (
            <p className="text-[10px] text-[#FFC107]">${stats.pendingCommissions.toFixed(2)} pending</p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="p-3 rounded-lg" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)' }}>
          ...
          <p className="text-lg font-bold text-[#FFD700]">${(stats?.depositBonusTotal ?? 0).toFixed(2)}</p>
        </div>
        <div className="p-3 rounded-lg" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)' }}>
      ...
      >
        <h4 className="text-sm font-bold text-white mb-3">Per-Level Breakdown</h4>
